...
def step(lane, arrival_rate):
    ...
    for i in range(0,lane_length - 2):
        if lane[i] == 1 and lane[i + 1] == 0 and lane[i + 2] == 0:  # Check no cars in adjacent sites
            new_lane[i] = 0  # Leave the current position empty
            new_lane[i + 1] = 1  # Move the car forward
            cars_moved += 1


    # Handle the next-to-last position 
#    if lane[lane_length - 2] == 1 and lane[lane_length-1] ==0:  # If there's no car after
    if lane[lane_length - 2] == 1 and lane[lane_length-1] ==0 and lane[0] ==0:  # If there's no car after
        new_lane[lane_length - 2] = 0  # Leave the current position empty
        new_lane[lane_length - 1] = 1  # Move the car forward
        cars_moved += 1

    # Handle the last position (cars exit the lane)
#    if lane[lane_length - 1] == 1:  # If there's a car at the last position 
    if lane[lane_length - 1] == 1 and lane[0] ==0 and lane[1] ==0:  # If there's a car at the last position 
